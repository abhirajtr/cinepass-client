...
const theatreOwnerApi = (() => {
    ...
    instance.interceptors.request.use(
        (config) => {
            ...
            const accessToken = state.authReducer?.theatreOwnerToken;
            ...
            if (error.response?.status === 401 && !originalRequest._retry) {
                ...
                try {
                    ...
                    const refreshResponse = await axios.post(
                        `${backendUrl}/theatreOwner/refresh-token`,
                        {},
                        { withCredentials: true }
                    );
                    ...
                    store.dispatch(setToken({ role: 'theatreOwner', token: newAccessToken }));
                    ...
                    store.dispatch(logout('theatreOwner'));
                    console.log('User logged out due to failed token refresh.');
                    window.location.href = '/theatreOwner/login';
                }
            } else {
